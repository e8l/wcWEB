...
let Method = (function (_BaseTask) {
  _inherits(Method, _BaseTask);

  function Method(selector, metadata) {
    _classCallCheck(this, Method);

    var _this = _possibleConstructorReturn(this, Object.getPrototypeOf(Method).call(this, selector, metadata));

    _this.target = "methods";
    _this.methodDef = [];
    return _this;
  }
  ...
  _createClass(Method, [{
      ...
      for (let elm of this.section) {
        if (elm.type !== "DefinitionList") {
          ...
          if (methodData.private) {
            ...
            let alt = this.metadata.ast.createHTMLElement(mustache.render(templates.get("method"), methodData));
    ...
    key: "analyzeMethodDeclaration",
    value: function analyzeMethodDeclaration(elm, methodData) {
    ...
    key: "addMethodDefinition",
    value: function addMethodDefinition(methodData, wrapper, lastCodeBlock, lastPos) {
      let declaration = "";
